...
public class Helper {
    ...
    public static boolean isCPF(Object value)
    {
        ...
        if(value instanceof Number)
        {
            return Helper.validaCPF(((Number) value).longValue());
        }

        String strValue = Formatter.onlyDigits(value.toString());
        return Helper.isEmptyString(strValue) ? false
                : Helper.validaCPF(Long.parseLong(strValue));
    }

    public static boolean isCPF(long value)
    {
        return Helper.validaCPF(value);
    }
    ...
    public static boolean isCNPJ(Object value)
    {
        ...
        if(value instanceof Number)
        {
            return Helper.validaCNPJ(((Number) value).longValue());
        }

        String strValue = Formatter.onlyDigits(value.toString());
        return Helper.isEmptyString(strValue) ? false
                : Helper.validaCNPJ(Long.parseLong(strValue));
    }

    public static boolean isCNPJ(long value)
    {
        return Helper.validaCNPJ(value);
    }
	...
	public static String retirarSinais(String str) {
	...
	public static double transformaValorPagoParaDouble(String preco) {
		double retorno = 0;
		String novoPreco = "";
		boolean encontrouVirgula = false;
		for (int i=0; i<preco.length(); i++) {
			char c = preco.charAt(i);
			String str = String.valueOf(c);
			if ( (str.equals(",")) && (!encontrouVirgula) ) {
				novoPreco = novoPreco + ".";
				encontrouVirgula = true;
			}
			if ( (Helper.isInteger(str)) && (!str.equals(".")) ){
				novoPreco = novoPreco + str;
			}
			if (Validator.isNotNull(novoPreco)) {
				retorno = Double.parseDouble(novoPreco);
			}
		}
	...
	public static String recuperarPrimeiroNome(String nome) {
		String retorno = null;
		
		if (Validator.isNotNull(nome)) {
			int pos = nome.indexOf(" ");
			retorno = nome.substring(0, pos);
	...
	public static String recuperarComplementoNome(String nome) {
		String retorno = null;
		
		if (Validator.isNotNull(nome)) {
			int pos = nome.indexOf(" ");
			retorno = nome.substring(pos).trim();
